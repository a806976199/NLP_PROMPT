...
\begin{frame}
    ...
    \eee{
        ...
        \item 相关文献的阅读.
        \item 修改MLC模型,创建一个数字加减乘除的任务,进行训练以及评估测试结果.
...
\begin{frame}
    ...
    \begin{itemize}
        \item 实现简单的加减乘运算的任务
        \item 任务数据集简介:\\
        \includegraphics[scale=0.2]{png/train.png}
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{模型结果说明}
    \begin{itemize}
        \item 目前做的只是10以内的加减乘的数学方法,通过一定的微调可以保证准确性\\
...
\subsection{lstm模型添加注意力以后的测试结果汇总}
\begin{frame}
    \begin{table}[ht]
        \centering
        \caption{准确率结果(模型)}        
