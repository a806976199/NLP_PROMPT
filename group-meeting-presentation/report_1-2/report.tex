...
\begin{frame}
    \frametitle{模型构建}
    \eee{
        \item 在原本的MLC模型基础上进行修改不太方便，有很多报错，所以我使用一个最简单的transformer模型，如此一来，参数量，注意力机制头数以及各种组合方式都可以自己控制，同时增加多个transformer模型合并也更好控制。目前这周就只完成了transformer模型的基本搭建,并进行了基本模型测试，还未把我们数据进行测试。
    }    
\end{frame}

\subsection{MathGPT模型构建思路}
\begin{frame}
    \frametitle{MathGPT模型}
    \eee{
        \item MathGPT测试特征\\
        \eee{
            \item 目前开源的代码那边测试的只有5位数以内的加减乘除，并没有进行高位数的测试,训练的也是五位数以内的数字。
        }
    }
\end{frame}
...
\subsection{文献阅读}
\begin{frame}
    \frametitle{纠正Transformer模型中推理过程中的多跳推理失败}
    \includegraphics[scale=0.3]{png/tblm-example.png}
    \eee{
        \item 直接无法得到答案，需要逐步执行获得最终结果。
        \item 这一步的输出可以作为下一步的输入操作。
    }
\end{frame}
...
\begin{frame}
    ...
    \eee{
        \item     
        \begin{itemize}
            \item \includegraphics[scale=0.3]{png/tblm.png}
        \end{itemize}            
        \item 这个模型的核心思想是通过在多头注意力层中注入相关记忆来提高预测的准确率。
    }
\end{frame}
...
\begin{frame}
    ...
    \eee{
        \item     
        \begin{itemize}
                        \item \includegraphics[scale=0.4]{png/tblm2.png}
        \end{itemize}            
        \item 这个模型的核心思想是通过在多头注意力层中注入相关记忆来提高预测的准确率。
    }
\end{frame}
