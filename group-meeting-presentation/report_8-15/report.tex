...
\begin{frame}
    \begin{figure}[ht]
        ...
        \subfigure[RevIN的测试结果,pass:100\%]{
            \includegraphics[width=0.45\textwidth]{png/revin.png}
        }
    \end{figure}
\end{frame}
...
\begin{frame}
	\frametitle{实验结论分析}	
	\begin{itemize}
        \item 使用RevIN后，每个数会变的很长，举个例子,本来是1,经过规范化以后就是0.2340001了。
        \item 如果均保留4位有效数字，就会导致对于实际相近的数据，经过归一化以后，都变成同一个值了。
        \item 对于RevIN的使用方法是否有问题？
	\end{itemize}
\end{frame}

\subsection{使用粗粒化的相关实验}

\begin{frame}
    \frametitle{关于预测长度的粗粒化测试}
    \begin{figure}[ht]
        \centering
        \subfigure[通过长度\ff{l}预测\ff{l},pass:36.51\%]{
            \includegraphics[width=0.4\textwidth]{png/test_a.png}
        }
        \hfill
        \subfigure[通过长度\ff{l}预测\ff{\frac{l}{2}},pass:97.5\%]{
            % \includegraphics[width=0.4\textwidth]{png/test_b.png}
            \includegraphics[width=0.4\textwidth]{png/test3-1.png}
        }
    \end{figure}
\end{frame}
...
\begin{frame}
    ...
    \begin{figure}[ht]
        \centering
        \subfigure[通过长度\ff{l}预测\ff{l},pass:4.3\%]{
            \includegraphics[width=0.4\textwidth]{png/output.png}
        }
        \hfill
        \subfigure[通过长度\ff{l}预测\ff{\frac{l}{2}},pass:75.5\%]{
            % \includegraphics[width=0.4\textwidth]{png/test_b.png}
            \includegraphics[width=0.4\textwidth]{png/test3-2.png}
        }
    \end{figure}
\end{frame}
...
\begin{frame}
	\frametitle{实验结论分析}	
	\begin{itemize}
        \item 即使使用粗粒化以后，预测长度变长以后对结果的精确度影响以及通过率影响都很大
        \item 修改的通过率低有一部分原因是语言模型预测长度有限导致的。
        \item 修改预测长度对通过的影响很大。
	\end{itemize}
\end{frame}


\subsection{混合多种类数据集测试}

\begin{frame}
    \frametitle{将现有的所有训练数据集随机选取部分混合进行测试}
    \begin{figure}[ht]
        \centering
        \subfigure[测试集数据,pass:37.14\%]{
            \includegraphics[width=0.4\textwidth]{png/initial1.png}
        }
        \hfill
        \subfigure[泛化性测试,pass:68\%]{
            \includegraphics[width=0.4\textwidth]{png/zero.png}
        }
    \end{figure}
\end{frame}
...
\begin{frame}
	\frametitle{实验结论分析}	
	\begin{itemize}
        \item 对于见过的任务结果比未见过的任务的通过率都要低，这个可能的原因是对于语言模型而言，对于数值越大的序列，预测的结果可能会于原本的数值差距很大，比如997,998,999，当语言模型可能预测出0.5，导致误差很大。
        \item 选取的数据集种类多，但是数量相对较少，所以预测结果不太精确。
	\end{itemize}
\end{frame}
