...
\begin{frame}
    ...
    \eee{
        ...
        \item 关于预测的准确度，主要做了以下几个测试：
        \eee{
            \item chatglm模型，历史节点：25
            \item lstm模型,使用随机的词嵌入向量，历史节点：25，对每只股票使用一个模型
            \item lstm模型,使用word2vec方法，历史节点：25，对每只股票使用一个模型
            \item lstm模型,使用word2vec方法，历史节点：100，对每只股票使用一个模型
            \item lstm模型,使用word2vec方法，历史节点：25，对所有股票使用单个模型
        }
        \item 使用Word2Vec方法对所有数据集中的字符进行训练以后，构造新的一些字符组合，并对每个字符的高维向量进行降维，然后进行可视化。
...
\begin{frame}
    \begin{table}[ht]
        ...
        模型& 平均准确率\\
        \hline
        随机初始化词向量，历史长度是25（多个模型） & 27.26\% \\
        \hline
        Word2Vec方法，历史数据点:25（多个模型） & {27.36\%}\\
        % 1
        \hline
        Word2Vec方法，历史数据点:100 (多个模型)& {27.40\%} \\
        % 8
        \hline
        Word2Vec方法，历史数据点:25（一个模型） & {29.49\%} \\
        \hline
        chatglm模型，历史数据点:25 & \red{32.34\%} \\
        \hline
    \end{tabular}
        \end{table}
\end{frame}
...
\begin{frame}
      \frametitle{单个字符可视化}
    \includegraphics[scale=0.4]{png/all_1.png}
\end{frame}

\begin{frame}
      \frametitle{两个字符所有组合可视化}
    \includegraphics[scale=0.4]{png/all_2.png}
\end{frame}

\begin{frame}
      \frametitle{两个字符组合中所有包含A的可视化}
    \includegraphics[scale=0.4]{png/all_A.png}
\end{frame}

\begin{frame}
      \frametitle{A,B,C,D的两两组合可视化（即收盘价低于开盘价的组合）}
    \includegraphics[scale=0.4]{png/all_A_B_C_D.png}
\end{frame}
...
\begin{frame}
	\frametitle{}
	\begin{itemize}
        \item 经过比较，对lstm模型而言，使用单个模型训练的效果更好。
        \item chatglm模型相比lstm模型的平均准确率更高。
        \item 使用随机的词向量和使用word2vec方法的词向量最终结果相差不大，使用word2vec方法的准确度略高一点。
        \item 经过可视化以后，使用增加了基本词向量以后，依旧可以发现CG,FB,AE等之间有密切的关联。
