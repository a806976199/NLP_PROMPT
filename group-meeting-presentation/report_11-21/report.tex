...
\begin{frame}
    ...
    \eee{
        \item 阅读了MLC模型的代码
        \item 修改了训练数据集的大小,测试模型的泛化能力对于训练数据量的要求,探索模型在少量数据的情况下模型的泛化能力.
        可以说明一下具体修改数据量的过程
        % 2 1000  batch 2  epoch 10(ok) 50(ok) 100(ok) 200
        % 20 ： 10000 batch 10 epoch 10(也差不多了) 50(ok) 100(ok)  200 
        % 200 ： 100000 batch 25 epoch 10  ok 
        % chatglm: 20 : 10000数据集 准确率 ok 
        \item 对比chatglm模型的任务泛化能力的测试
    }
\end{frame}
\subsection{阅读MLC模型代码}
\begin{frame}
    \frametitle{MLC模型的一些特点说明}
    \begin{itemize}
        \item  MLC模型相比传统训练的模型的特殊之处在于训练的时候,对于每个训练数据都给出了一个相应的规则.如图:\\
        \includegraphics[scale=0.25]{png/regular.png}
    \end{itemize}
\end{frame}

\subsection{chatglm模型和MLC模型泛化能力对比}
\begin{frame}
    \frametitle{探究MLC模型泛化能力}
    \begin{itemize}
        \item 探究数据量对MLC模型泛化能力的影响
        \eee{
            \item 通过使用1000,10000,100000个txt数据文件分别探究模型泛化能力的比较.
            \item 对每个数据量进行训练,并进行泛化性测试,得到如下表的结果准确率.
        }
        \item MLC模型与chatglm的泛化能力对比
        \eee{
            \item MLC模型和chatglm模型使用相同大小的数据集,均使用10000个txt文本数据集(chatglm模型没有使用额外的GRAMMAR提示信息)
        }
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{table}[ht]
        ...
        \hline
        模型及参数& 准确率(验证集) & 准确率(泛化) \\
        \hline        
        data:1000,nepochs:10& {7.14\%} & {0\%}\\
        \hline
        data:1000,nepochs:100& {7.79\%} & {0\%}\\
        \hline
        data:1000,nepochs:200& {9.14\%} & {0.5\%}\\
        \hline
        data:10000,nepochs:10& {7.0\%} & {0.0\%}\\
        \hline
        data:10000,nepochs:100& {100\%} & {57.2\%}\\
        \hline
        data:10000,nepochs:200& {100\%} & {72.2\%}\\
        \hline
        data:100000,nepochs:10& {100\%} & {71.7\%}\\
        \hline
        chatglm,data:10000 &- & {3.21\%}\\
        \hline
    \end{tabular}
        \end{table}
\end{frame}
...
\begin{frame}
    \frametitle{结论}
    \eee{
        \item chatglm语言模型在逻辑任务上的泛化表示不好,而且其中的3\%完成的只是最基本的一对一问题.
        \item MLC模型在足够多epochs条件下,能够得到很强的泛化性,基本上和chatglm喂养相同的数据集,MLC模型对于泛化任务能够达到70\%的准确率,而chatglm只有3\%,几乎就是没有泛化能力.
        \item MLC模型可以使用更少量的数据集,训练时使用更多的epochs也可以达到不错的准确率.
        \item 从代码中可知MLC模型结构比较简单,在单种任务上有很不错的泛化效果,但是对于多种任务的泛化能力上表现还是有待提高!
    }
\end{frame}
...
\begin{frame}
    \frametitle{下一步计划}
    \eee{
        \item 做其他的实验,确保他的这种框架在类似任务上同样有效.
        \item chatglm的prompt改成和MLC模型中类似的提示数据集.
    }
\end{frame}
