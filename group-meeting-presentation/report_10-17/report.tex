...
\begin{frame}
    ...
    \eee{
        ...
        \item 关于FinGLM问答系统的技术框架
        \item 关于预测，这周主要做了以下几个测试：
        \begin{itemize}
            % \item chatglm模型，历史节点：25
            % \item lstm模型,使用随机的词嵌入向量，历史节点：25，单一模型建模
            % \item lstm模型,使用word2vec方法，历史节点：25，单一模型建模
            % \item lstm模型,使用word2vec方法，历史节点：100，单一模型建模
            % \item lstm模型,使用word2vec方法，历史节点：25,共享模型建模
            % \item lstm模型添加注意力机制后的结果,历史节点：25   
            \item transformer模型,历史节点:25,单一模型建模
            \item transformer模型添加lstm,历史节点:25,单一模型建模
            \item lstm添加注意力的模型，历史节点:25,单一模型建模
            \item transformer模型,历史节点:25,共享模型建模
            \item lstm添加注意力的模型，历史节点:25,单一模型建模
            \item transformer模型添加lstm,历史节点:25,单一模型建模
        \end{itemize}
        \item 使用Word2Vec方法对所有数据集中的字符进行训练以后，构造新的一些字符组合，并对每个字符的高维向量进行降维，然后进行可视化。其中按照频率的顺序，把高频率的放在最前面        
    }
\end{frame}
...
\begin{frame}
    \frametitle{任务简介}
    \includegraphics[scale=0.4]{png/target.png}
\end{frame}

\begin{frame}
    \frametitle{解决问题示例}
    \includegraphics[scale=0.37]{png/example.png}
\end{frame}
...
\begin{frame}
    \begin{table}[ht]
        ...
        \hline
        LSTM引入注意力机制，单一模型建模 & {32.73\%} \\
        \hline
        LSTM引入注意力机制，共享模型建模 & \red{34.56\%} \\
        \hline
        transformer模型，单一模型建模 & {33.61\%} \\        
        \hline
        transformer模型，共享模型建模 & {34.55\%} \\
        \hline
        Transformer-LSTM联合模型，单一模型建模 & {32.93\%} \\
        \hline
        Transformer-LSTM联合模型，共享模型建模 & {31.61\%} \\
        \hline
    \end{tabular}
        \end{table}
\end{frame}
...
\begin{frame}
    \frametitle{两字符组中数量占比前10的可视化结果}
    \includegraphics[scale=0.38]{png/most_10.png}
\end{frame}

\begin{frame}
    \frametitle{两字符组中数量占比前20的可视化结果}
    \includegraphics[scale=0.38]{png/most_20.png}
\end{frame}

\begin{frame}
    \frametitle{两字符组中数量占比前30的可视化结果}
    \includegraphics[scale=0.38]{png/most_30.png}
\end{frame}
...
\begin{frame}
	\frametitle{}
	\begin{itemize}
        \item 共享模型都准确度基本都高于单一模型的准确度
        \item LSTM模型引入注意力机制以后明显准确度有明显的上升，且超越了chatglm模型都准确度 
        \item 在最高最低价接近的时候，开盘价和收盘价之间的变化紧密相关
        \item 从馒头科技的实验成果中可以看出语言模型都潜力很大，确实能够解决一些实际问题
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
        \item 仔细查看FinGLM代码，学习如何联系数据库来完成检索任务
        % \item 
