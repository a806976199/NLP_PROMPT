...
% \begin{frame}
    % \frametitle{}
    % \begin{itemize}
        % \item 在任务多样性比较高的任务上PT模型表现明显优于贝叶斯分类器,且和在此任务最优估计模型性能差不多.
        % \item 确定了上下文理解能力的任务数量阈值,低于此阈值，增加预训练数据集，同时保持任务多样性固定，会使PT偏向于预训练任务分布。相反，超过这个阈值，在不增加任务多样性的情况下增加数据集大小可以提高PT在新的、未见过的任务上的性能。
        % 这表明PT的行为在每个任务的许多示例的限制下经历了急剧的算法相变，与阈值之前的 TP和阈值之后的TP上的最佳估计器保持一致.
        % \item 在固定SNR下增加任务维度会增加任务多样性的阈值.然而,PT误差随维度的缩放比例远远优于TP的最佳贝叶斯估计量,当任务多样性超出我们考虑的所有维度的阈值时，随着维度的增加，PT保持接近最优估计器，而TP的最优估计量逐渐与最优估计量变得越来越不相似。
        % \item 增加权重衰减会降低任务多样性阈值，而增加层数或嵌入大小会增加任务多样性阈值。这阐明了正则化和模型容量对ICL出现的影响.
    % \end{itemize}
% \end{frame}

\begin{frame}
    \frametitle{Pretrained transformers(PT模型)}
    \includegraphics[scale=0.2]{png/example1.png}
    \eee{
        \item 探究PT模型在不同任务数时在泛化任务和预训练数据集上的表现.
        \item 通过固定任务种类,增加同一任务的数量,探究阈值的变化.
        \item 在超过阈值以后,任务种类数更大能够使得PT与最优估计器更加接近.
    }
\end{frame}

\begin{frame}
    \frametitle{实验2}
    \includegraphics[scale=0.2]{png/study3.png}\\
    我们还通过将训练步骤数从500K增加到1M，同时将批量大小固定为256，在每个任务多样性下增加每个任务的epochs。
    \eee{
        \item 训练步数越高，在任务数少的时候估计效果不佳，可能是出现了过拟合，任务多的时候，相对效果较好。
    }
\end{frame}
...
\subsection{MLC模型在解决简单加减乘计算的能力展示}
\begin{frame}
    \frametitle{模型部署说明}
    \begin{itemize}
        \item 模型实现功能:能够对数字完成基本的加减乘以及混合运算.
        ...
        \item 我们模型主要能够实现以下的功能:\\
            我们的模型能够对数字进行加减乘及混合运算的规则进行学习，然后进行其他相关数值的计算。\\
            比如我们的模型学习3+5-4的学习，这种混合运算的模型，那么对于所有x+y-z都能以极高的准确率进行问题的回答。
        % \item 模型的一些规律:\\
        % \eee{
...
\begin{frame}
    ...
    \begin{itemize}
        \item 加减法计算结果\\
        \includegraphics[scale=0.24]{png/mlc_result2.png} 
        \item 混合运算结果(加法，减法，乘法)\\
        \includegraphics[scale=0.24]{png/mlc_result1.png} 
        \item 详细结果见代码可以运行得到
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{下一步计划}
    \eee{
        \item 将简单的加减乘任务尝试在数值上更加复杂,探究其泛化能力.
        \eee{
            \item 更复杂的相关问题()
            \item 长度设置一下()
            \item 无法解决4个sum的问题。
        }
        \item 更换注意力机制的变体以及在不同编码器上进行测试查看效果.
    }
\end{frame}
