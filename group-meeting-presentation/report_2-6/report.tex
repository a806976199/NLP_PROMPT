...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \eee{
            ...
            \item experiments
            \eee{
                \item 目前实现10进制的任意五位数以内的加减运算能够做到100\%正确率(已完成)
                % \item 完成任意10位数的加减运算
                \item 针对小数减大数的问题进行比较测试(已完成)
                \item 对于加法中需要用到较多进位情况进行结果比对(已完成)
                \item 对于减法中需要用到较多进位情况进行结果比对(已完成)
                % \item 使公认的数学计算数据集进行测试
                \item 考虑规则在其他方面上的应用，比如考虑求导，积分等具有较强规则性的应用。(未完成)
            }
        }
        \item 代码进度
        \eee{
            \item 大致实现各种位数的加减算法(已完成)
            \item 实验数据以及测试结果总结(已完成)
            \item 做其他方面的应用
        }
        % \item 遇到的问题
        % \eee{
...
\begin{frame}
    \frametitle{张量积计算，加减计算(混合规则)}
    \eee{
        \item 功能:我们的模型能够同时处理两位数的加法,减法以及两个向量的外积
        \item 例:\\
        \includegraphics[scale=0.22]{png/lomixule.png}
        % \item 
    }
\end{frame}
\begin{frame}
    \frametitle{高位数的加减计算}
    \eee{
        \item 功能:作为一个语言模型，能够在不借助外部工具的情况下计算10位数以内的加法，减法等。
        \item 高位减法例:\\
        \includegraphics[scale=0.22]{png/sub_rule-h.png}
        \item 高位加法例:\\
        \includegraphics[scale=0.22]{png/sum_rule-h.png}
    }
\end{frame}
...
\begin{frame}
    \frametitle{添加数据说明:}
    \eee{
        \item 添加当前热门模型以及我们自己的语言模型的低位数的加减结果。        
        \item 添加当前热门模型以及我们自己的语言模型的高位数的加减结果。
        \item 展示我们模型遵循规则的能力，即能同时实现张量积和数字的加减计算。
        \item 测试普通随机数字计算数据集合以及和我们构造的特定数据集进行比较，从而说明我们的模型在计算数字上能力更加的全面以及精确。
    }
\end{frame}

\begin{frame}
    \frametitle{关于无法使用无限长度规则的计算原因}
    \eee{
        \item token数是有限的。
        \item 对于一个字符串比如‘1,3,4,7,5,5,7,5,5,3，3’，‘4,3,4,7,5,5,7,5,5,3，3’而言，计算过程是必须要对齐的，无线长度是无法处理的。
        \item 代码是需要借助for循环，语言模型并不可以这么处理。
    }
\end{frame}
...
% \begin{frame}
%     \frametitle{整理了一些常见语言模型的一些计算结果}
%     用我们的模型以及这些模型进行对比计算:43423-32424
%     \begin{itemize}
%         \item llama-2-7b:11001
%         \item llama-2-13b:11001
%         \item llama-2-70b:-9001
%         \item Claude-instant-100k:10939
%         \item Google-PaLM:11001
%         \item \red{chatgpt3.5:10999}
%         \item Qwen-72b-Chat:11009
%         \item Mistral-Medium:77595
%         \item \red{MathRuleGLM-8k:10999}
%     \end{itemize}
% \end{frame}
...
\begin{frame}
    \frametitle{模型相关信息}
    \begin{itemize}
        \item 对于一个k进制的n位加减法需要的复合规则和子规则数
        \eee{
        \item 子规则数:\ff{4k^2}
        \item 进位规则数:\ff{2^{n+1}}
        \item 对齐规则数:\ff{2n^2}
