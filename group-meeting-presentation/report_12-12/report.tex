...
\subsection{MLC模型在解决大数值上加减乘计算能力}
\begin{frame}
    \frametitle{模型部署说明}
    \begin{itemize}
...
\begin{frame}
    \frametitle{100以内加减的设计思路}
    \begin{itemize}
        \item 使用10进制进行计算，无需增加新的规则，只需理解1到9即可，比如34,可以理解看成10 10 10 1 1 1 1 。
        \item 同理类推，对于大数都可以用类比思路，这样的思路之下，只需要教模型理解数字1到9即可，所有数字都可以进行类型的加减了。
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{table}[ht]
        ...
        四则算法种类 & 数据量 & 训练次数 & 准确率  \\
        \hline        
        10以内加减混运算 & {5k} & {200} & {100\%} \\
        \hline
        10以内加减乘混运算 &  {5k} & {200} & {96.83\%} \\
        \hline
        10以内加减混运算 & {100k} & {21} & {76.5\%} \\    
        \hline
        10以内加减乘混运算 & {100k} & {21} & {55.67\%} \\
        \hline
        100以内的加减混运算 & {100k} & {20} & {75\%} \\
        \hline
        100以内的加减混运算 & {25k} & {11} & {87.5\%} \\
        \hline
    \end{tabular}
        \eee{
            % \item 分别在解码器和编码器中引入注意力机制，但是最终效果都不如不加入注意力机制。
            % \item 加入注意力机制以后变得更加相比没加之前稳定性有变差.
            \item 可以使用较少的数据量在训练次数上足够多，就可以以高准确率来完成计算任务。
            % \item 从数据结果来看，这个注意力机制对整体的改变影响不大，改进的gru模型由于参数减少使得结果准确度还有所下降。
...
\begin{frame}
    \frametitle{下一步计划}
    \eee{
        \item 将任务在数值上更加复杂,探究其泛化能力.
        \eee{
            % \item 更复杂的相关问题()
            % \item 长度设置一下()
            % \item 无法解决4个sum的问题。
            \item 将数值进一步的提高，理论上可以进行任何数值的加减计算。
            \item 开始准备高位数的乘法运算模型搭建。
        }
        % \item 使用
    }
\end{frame}
