...
\begin{frame}
    ...
    \eee{
        ...
        \item chatglm模型:使用合并的训练集数据进行训练，并对测试集进行相应的测试。
        \item LSTM模型:进行训练的时候选择了单支股票测试结果和多只股票数据合并的两种训练集，并且测试同一组数据结果并进行比较。
        % 我将这个100个数据集合合并然后进行测试一下，然后就是
        \item 将美股数据的k线图分别用A,B,C,..,L这12个字母表示，并使用char\_tensor的方法将字符文本映射到对应的目标字符上。
        \item 使用Word2Vec方法对所有数据集中的字符进行训练以后，并对每个字符的高维向量进行降维，然后进行可视化。
        % 展示的几个数据集合，以及整体的1000个股票合并的结果的结果，还有就是那单独6个的结果。
        % \item 准确度结果数据详见excel表格
    }
\end{frame}
...
\begin{frame}
    \begin{table}[ht]
        ...
        模型& lstm & chatglm \\
        \hline
        AGR & 0.2254 & \red{0.4405}  \\
        % 7
        \hline
        AMD & \red{0.3710} & {0.205} \\
        % 1
        \hline
        BAH & 0.3185 & \red{0.4045} \\
        % 8
        \hline
        BKR & \red{0.20} & {0.115} \\
        \hline
        IGF & {0.1053} & \red{0.3172} \\
        % 10
        \hline
    \end{tabular}
        \end{table}
...
\begin{frame}
    \frametitle{\tiny AGR股票}    
    \begin{figure}[ht]
        \centering
        \subfigure[chatglm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/AGR_chatglm.png}
        }
        \hfill
        \subfigure[lstm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/AGR_lstm.png}
        }
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{\tiny AMD股票}    
    \begin{figure}[ht]
        \centering
        \subfigure[chatglm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/AMD_chatglm.png}
        }
        \hfill
        \subfigure[lstm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/AMD_lstm.png}
        }
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{\tiny BAH股票}    
    \begin{figure}[ht]
        \centering
        \subfigure[chatglm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/BAH_chatglm.png}
        }
        \hfill
        \subfigure[lstm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/BAH_lstm.png}
        }
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{\tiny BKR股票}    
    \begin{figure}[ht]
        \centering
        \subfigure[chatglm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/BKR_chatglm.png}
        }
        \hfill
        \subfigure[lstm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/BKR_lstm.png}
        }
    \end{figure}
\end{frame}

\begin{frame}
    \frametitle{\tiny IGF股票}    
    \begin{figure}[ht]
        \centering
        \subfigure[chatglm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/IGF_chatglm.png}
        }
        \hfill
        \subfigure[lstm测试结果]{
            \includegraphics[width=0.45\textwidth]{png/IGF_lstm.png}
        }
    \end{figure}
\end{frame}
...
\begin{frame}
    \includegraphics[scale=0.4]{png/total.png}
\end{frame}
\begin{frame}
...
\begin{frame}
	\frametitle{}
	\begin{itemize}
        \item 经过比较，lstm模型和chatglm模型的结果差不多，但是chatglm模型总体来说比较稳定。
        % 但是lstm的结果在单个数据集上的测试结果大部分是一条直线，就像是对数据集合用了一条直线进行最佳拟合。
        \item 通过使用Word2Vec可视化发现，k线图预测中最高价和最低价对最终预测结果的影响比较大。
