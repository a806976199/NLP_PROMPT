...
#                inputs.append(Input[:-1])
                my_str = ""
                for k in range(len(my_list)):
                    my_str += "dim:"+my_list[k][:-1]+','
                inputs.append(my_str[:-1])
                Input = ""
                ...
                future_list = [float(value) for value in Output[:-1].split(',')]
                history_list = [float(value) for value in my_list[-1][:-1].split(',')]
                #最终的输出结果:
                tot_res=""
                for state in range(1,pre_num):
                    tot_res += "step"+str(state)+":"+str(history_list[-pre_num + state:])[1:-1]+','+str(future_list[:state])[1:-1]+",\n,"
                tot_res += "result:"+str(future_list)[1:-1]
                #outputs.append(Output[:-1])
                outputs.append(tot_res)
                Output = ""
        return inputs,outputs
...
#            print(i)
    ...
    os.remove("test.csv")
    import shutil
    pwd= '/home/ckj/file/temp-experience/NLP_PROMPT/project_root/code/tot/'
...
i = 0
for struct_num in range(8,10,2):
