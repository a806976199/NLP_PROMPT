...
file, file_len = data_utils.get_data(args.filename)
...
try:
    ...
    for my_str in predict_data:
        res = generate(decoder,prime_str = my_str[:-1])
        result['predict'].append(data_utils.letter_to_number.get(res[-1]))
        result['labels'].append(data_utils.letter_to_number.get(my_str[-1]))
    #     if res[-1] == my_str[-1]:
    #         sum += 1.0
    # print(round(sum/(file_len*0.2),4))
    save_file = 'new-save/'+args.filename[12:-4]+'.json'
